...
export default function Patients() {
    ...
    function searchPatients() {
        fetch(
            `${import.meta.env.VITE_API_URL}/api/patients?first_name=${firstName}&last_name=${lastName}&dob=${dob}&patient_id=${patientId}`,
            {
                method: "GET",
                cache: "no-store",
                headers: {
    ...
    function addToWaitlist() {
        console.log("reason:", reason, "preferredDoctor:", preferredDoctor);

        if (reason === "" || preferredDoctor === "") {
            ...
            {showWaitlistModal && (
                <div className="fixed z-50 inset-0 bg-black/50 flex items-center justify-center px-4">
                    <div className="bg-white rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        ...
                        <button className="bg-[#7AAE9E] text-white px-3 py-1 rounded-lg" onClick={addToWaitlist}>Submit</button>
                    </div>
                </div>
            )}
